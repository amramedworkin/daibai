...
def load_config(config_path: Optional[Path] = None, env_path: Optional[Path] = None) -> Config:
    ...
    # Inject standard shared databases from DB_RUNTIME_* when available.
    ...
    if runtime_host:
        ...
        for shared_db in ["chinook", "northwind"]:
            if shared_db not in databases:
                databases[shared_db] = DatabaseConfig(
                    name=shared_db,
                    type="mysql",
                    host=runtime_host,
                    port=runtime_port,
